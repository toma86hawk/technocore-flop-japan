...
SEED = 20260920158
random.seed(SEED)
# uniform random, same method as r147..r156
...
for i in idx:
    p = q[i]
    w = p["worker"]
    if seen_worker.get(w, 0) >= 2:      # cap 2 per worker so 15 verdicts are not one key
        continue
    seen_worker[w] = seen_worker.get(w, 0) + 1
    picked.append(p)
    if len(picked) == 15:
        break
json.dump(picked, io.open("guide/_r158_picked.json", "w", encoding="utf-8"), ensure_ascii=False)
