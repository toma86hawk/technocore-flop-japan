...
def export(room, tag, timeout=300):
    ...
    open("_r118_" + room + ".jsonl", "w", encoding="utf-8").write(raw)
    out = []
...
print("r118 falsifier run (round 118)", now.isoformat())
...
json.dump(roster_now, open("_r118_roster.json", "w"), indent=0)
...
roster_110 = json.load(open("_r110_roster.json", encoding="utf-8"))
print("  r108 %d -> r109 %d -> r110 %d -> r118 %d  (+%d since r110)"
      % (len(roster_108), len(roster_109), len(roster_110), len(roster_now), len(roster_now) - len(roster_110)))
print("  r110 roster is a SUBSET of today's: %s (dropped %d)"
      % (set(roster_110) <= set(roster_now), len(set(roster_110) - set(roster_now))))
